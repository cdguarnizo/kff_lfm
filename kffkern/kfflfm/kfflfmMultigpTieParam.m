function tieInd = kfflfmMultigpTieParam(model, options)
...
for i = 1:model.nout
    tieInd{end+1} = paramNameRegularExpressionLookup(model, ['. kfflfm ' num2str(i)  ' spring']);
    tieInd{end+1} = paramNameRegularExpressionLookup(model, ['multi ' ...
        '[0-9]+ kfflfm ' num2str(i)  ' damper']);
end