...
fd = amc_to_matrix('../datasets/CMUmocap/64_01.amc');

% Remove the following channels
IndDel = [25,26,34,37,38,46]; %48
fd(:,IndDel) = [];
...
if strcmp(options.kernType(1:2),'kf')
    %name = strcat('temp/MOCAP_swing_',options.kernType,'S',...
    %    num2str(options.kern.S),'_NLF',num2str(options.nlf),'.mat'); 
    name = strcat('temp/MOCAP_swing_',options.kernType,'S',...
        num2str(options.kern.S),'.mat');
else
    if options.nlf ~= 6
        name = strcat('temp/MOCAP_swing_',options.kernType,'_NLF',...
            num2str(options.nlf),'.mat');
    else
        name = strcat('temp/MOCAP_swing_',options.kernType,'.mat');
    end
end
