function [Kff, Kfu, Kuu] = kffsimglobalKernCompute(kern, outX, latX, gamma)
    ...
    if nargin < 3,
        ...
        for q = 1:kern.nlf,
            ...
            for d = 1:kern.nout,
                ...
                Kfft{d,d} = kern.funcNames.computeOut(kernOut, outX{d});
                for dp = d+1:kern.nout,
                    kernOut2.decay = kern.decay(dp);
                    kernOut2.sensitivity = kern.sensitivity(dp,q);
                    Kfft{d,dp} = kern.funcNames.computeCrossOut(kernOut, kernOut2, outX{d}, outX{dp});
                    Kfft{dp,d} = Kfft{d,dp}.';
                end
            end
        ...
        Kfft = cell(kern.nout, kern.nout);
        Kff = zeros(model.N,size(cell2mat(latX),1));
        kernOut = kern.template.output;
        kernOut2 = kern.template.output;
        for q = 1:kern.nlf,
            kernOut.inverseWidth = kern.inverseWidth(q);
            ...
            for d = 1:kern.nout,
                kernOut.decay = kern.decay(d);
                kernOut.sensitivity = kern.sensitivity(d,q);
                %Kfft{d,d} = kern.funcNames.computeOut(kernOut, outX{d}, latX{d});
                for dp = 1:kern.nout,
                    kernOut2.decay = kern.decay(dp);
                    kernOut2.sensitivity = kern.sensitivity(dp,q);
                    Kfft{d,dp} = kern.funcNames.computeCrossOut(kernOut, kernOut2, outX{d}, latX{dp});
                end
            end
            Kff = Kff + cell2mat(Kfft);
        end
