function [model, options, params] = ftcmgpOptimise(model, display, iters)
...
params = ftcmgpExtractParam(model); 
options = optOptions;
if display
    options(1) = 1;
...
if strcmp(func2str(optim), 'optimiMinimize')
  ...
  params = optim('ftcmgpObjectiveGradient', params, options, model);
elseif strcmp(func2str(optim), 'rprop')
    params = optim(params, 'ftcmgpObjectiveGradient', iters, model);    
else
  % NETLAB style optimization.
  [params, options] = optim('ftcmgpObjective', params,  options, ...
                 'ftcmgpGradient', model);
end
