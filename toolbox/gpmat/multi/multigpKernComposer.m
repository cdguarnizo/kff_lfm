...
switch type
    ...
    case 'kffgg', % The diffusion kernel.
        if isfield(options, 'kern') && ~isempty(options.kern)
            kernType{specLatent+1} = {'parametric', options.kern, 'kffgaussian'};
        else
            kernType{specLatent+1} = 'kffgaussian';
        end
        
        for i = 1:limit
            if isfield(options, 'kern') && ~isempty(options.kern)
                kernType{i+numLatent+1} = {'parametric', options.kern, 'kffgg'};
            else
                kernType{i+numLatent+1} = 'kffgg';
            end
        end
    ...
    case 'kfflfm', % the 2nd order differential equation.

        if isfield(options, 'kern') && ~isempty(options.kern)
            kernType{specLatent+1} = {'parametric', options.kern, 'kffrbf'};
        else
            kernType{specLatent+1} = 'kffrbf';
        end

        for i = 1:limit
            if isfield(options, 'kern') && ~isempty(options.kern)
                kernType{i+numLatent+1} = {'parametric', options.kern, 'kfflfm'};
            else
                kernType{i+numLatent+1} = 'kfflfm';
            end
        end
    ...
    case 'kffsim' % The 1st order differential equation.
        ...
        for i = 1:limit
            if isfield(options, 'kern') && ~isempty(options.kern)
                kernType{i+numLatent+1} = {'parametric', options.kern, 'kffsim'};
            else
                kernType{i+numLatent+1} = 'kffsim';
            end
        end
